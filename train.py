...
class Train(object):
    ...
    def train_batch_RL(
        ...
        """
        s_t = enc_hidden  # [200,512] ,[200,512]                                                                           #Decoder hidden states
        x_t = get_cuda(
            T.LongTensor(len(enc_out)).fill_(self.start_id)
        )  # Input to the decoder
        prev_s = None  # Used for intra-decoder attention (section 2.2 in https://arxiv.org/pdf/1705.04304.pdf)
        ...
        for t in range(config.max_dec_steps):
            x_t = self.model.embeds(
                x_t
            )  # [200,256] note that the embedding size is 256
...
if __name__ == "__main__":
    ...
    parser.add_argument("--load_model", type=str, default="0025000.tar")
    ...
    print(
        "intra_encoder:", config.intra_encoder, "intra_decoder:", config.intra_decoder
    )
