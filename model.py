...
class Decoder(nn.Module):
    ...
    def forward(self, x_t, s_t, enc_out, enc_padding_mask, ct_e, extra_zeros, enc_batch_extend_vocab, sum_temporal_srcs, prev_s):
        ...
        Args:
            x_t (_type_): encoder embedding 
            s_t (_type_): encoder hidden
            enc_out (_type_): Outputs of the encoder for all time steps (batch_size, length_input_sequence, 2*hidden_size)
            enc_padding_mask (_type_): Mask for encoder input; Tensor of size (batch_size, length_input_sequence) with values of 0 for pad tokens & 1 for others
            ct_e (_type_): encoder context vector for time_step=0
            extra_zeros (_type_): Tensor used to extend vocab distribution for pointer mechanism
            enc_batch_extend_vocab (_type_): Input batch that stores OOV ids
            sum_temporal_srcs (_type_): I don't know what it is 
            prev_s (_type_): I don't know what it is 
        ...
        dec_h, dec_c = s_t #two units all belong to [batch_size, hidden_size]
        st_hat = T.cat([dec_h, dec_c], dim=1) # [batch_size, 2 * hidden_size]
        ...
        p_gen = T.cat([ct_e, ct_d, st_hat, x], 1) # it seems that concat them to [batch_size, hidden_dim * 5 + emb_dim,]
        p_gen = self.p_gen_linear(p_gen)            # bs,1
        p_gen = T.sigmoid(p_gen)                    # bs,1 something like a gate

        out = T.cat([dec_h, ct_e, ct_d], dim=1)     # bs, 4*n_hid
        out = self.V(out)                           # bs,n_hid
        out = self.V1(out)                          # bs, n_vocab make it to [batch_size, vocab_size]
        vocab_dist = F.softmax(out, dim=1) # then it is [batch_size]
        vocab_dist = p_gen * vocab_dist # something like a gate multiply
