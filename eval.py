...
class Evaluate(object):
    def __init__(self, data_path, opt, batch_size=config.batch_size):
        ...
        self.batcher = Batcher(
            data_path, self.vocab, mode="eval", batch_size=batch_size, single_pass=True
        )
        self.opt = opt
        time.sleep(5)
    ...
    def evaluate_batch(self, print_sents=False):
        ...
        if print_sents:
            self.print_original_predicted(
                decoded_sents, ref_sents, article_sents, load_file
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--task", type=str, default="validate", choices=["validate", "test"]
    )
