...
class Beam(object):
    def __init__(self, start_id, end_id, unk_id, hidden_state, context):
        ...
        self.scores[0][
            0
        ] = 0  # At time step t=0, all beams should extend from a single beam. So, I am giving high initial score to 1st beam
    ...
    def advance(self, prob_dist, hidden_state, context, sum_temporal_srcs, prev_s):
        ...
        beams_order = T.div(
            best_scores_id.squeeze(1), n_extended_vocab, rounding_mode="trunc"
        )  # (beam,); sorted
    ...
    def get_best(self):
        best_token = (
            self.tokens[0].cpu().numpy().tolist()
...
def beam_search_pl(
    ...
    n_rem = (
        batch_size  # Index of beams that are active, i.e: didn't generate [STOP] yet
    )
    sum_temporal_srcs = (
        None  # Number of examples in batch that didn't generate [STOP] yet
    )
    prev_s = None
    ...
    for t in range(config.max_dec_steps):
        ...
        if sum_temporal_srcs is not None:
            sum_temporal_srcs = (
                ...
                .contiguous()
                .view(-1, enc_out.size(1))
            )  # rem*beam, n_seq

        if prev_s is not None:
            prev_s = (
                T.stack([beam.prev_s for beam in beams if beam.done == False])
                .contiguous()
                .view(-1, t, config.hidden_dim)
            )  # rem*beam, t-1, n_hid
        ...
        enc_out_beam = (
            enc_out[beam_idx]
            .view(n_rem, -1)
            .repeat(1, config.beam_size)
            .view(-1, enc_out.size(1), enc_out.size(2))
        )
        enc_pad_mask_beam = (
            enc_padding_mask[beam_idx]
            .repeat(1, config.beam_size)
            .view(-1, enc_padding_mask.size(1))
        )

        extra_zeros_beam = None
        if extra_zeros is not None:
            extra_zeros_beam = (
                extra_zeros[beam_idx]
                .repeat(1, config.beam_size)
                .view(-1, extra_zeros.size(1))
            )
        enc_extend_vocab_beam = (
            enc_batch_extend_vocab[beam_idx]
            .repeat(1, config.beam_size)
            .view(-1, enc_batch_extend_vocab.size(1))
        )

        final_dist, (dec_h, dec_c), ct_e, sum_temporal_srcs, prev_s = decoder(
            x_t,
            s_t,
            enc_out_beam,
            enc_pad_mask_beam,
        ...
        if prev_s is not None:
            prev_s = prev_s.view(
                n_rem, config.beam_size, -1, config.hidden_dim
            )  # rem, beam, t
