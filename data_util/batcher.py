...
class Batcher(object):
    ...
    def __init__(self, data_path, vocab, mode, batch_size, single_pass):
        ...
        if single_pass:
            self._num_example_q_threads = (
                1  # just one thread, so we read through the dataset just once
            )
            self._num_batch_q_threads = 1  # just one thread to batch examples
            self._bucketing_cache_size = 1  # only load one batch's worth of examples before bucketing; this essentially means no bucketing
            self._finished_reading = (
                False  # this will tell us when we're finished reading the dataset
            )
        else:
    ...
    def fill_example_queue(self):
        ...
        while True:
            ...
            example = Example(
                article, abstract_sentences, self._vocab
